...
void AEnemyController::Tick(float DeltaSeconds)
{
	...
	switch (CurrentState)
	{
	case IDLE: OnIdleTick(DeltaSeconds);
		break;
	case PATROL: OnPatrolTick(DeltaSeconds);
		break;
	case CHASE: OnChaseTick(DeltaSeconds);
		break;
	}
}
...
void AEnemyController::OnMoveCompleted(FAIRequestID RequestID, const FPathFollowingResult& Result)
{
	...
	if (CurrentState == PATROL)
	{
		if (PossessedEnemy)
		{
			NavigationSystem->GetRandomPointInNavigableRadius(PossessedEnemy->GetActorLocation(), PatrolRange,
			                                                  PatrolTarget);
...
void AEnemyController::ChangeState(EnemyState NewState)
{
	...
	switch (CurrentState)
	{
	case IDLE: OnIdleExit();
		break;
	case PATROL: OnPatrolExit();
		break;
	case CHASE: OnChaseExit();
		break;
	}

	switch (NewState)
	{
	case IDLE: OnIdleEnter();
		break;
	case PATROL: OnPatrolEnter();
		break;
	case CHASE: OnChaseEnter();
		break;
	}
...
void AEnemyController::OnPatrolTick(float DeltaTime)
{
	if (PossessedEnemy)
	{
		if (FVector::Dist(PossessedEnemy->Player->GetActorLocation(), PossessedEnemy->GetActorLocation()) <
			DetectionRange)
		{
			ChangeState(CHASE);
			return;
...
void AEnemyController::OnChaseTick(float DeltaTime)
{
	if (PossessedEnemy)
	{
		...
		if (TowardsPlayer.Size() <= DetectionRange) { MoveToLocation(PossessedEnemy->Player->GetActorLocation()); }
		else
		{
			StopMovement();
			ChangeState(PATROL);
