	...
	if (TargetActor)
	{
		...
		if (!bStartingSpotlight)
		{
			...
			if (Enemy)
			{
				...
				SetActorRotation(TargetDirection.Rotation());
				Spotlight->SetLightColor(FLinearColor::LerpUsingHSV(HealthyColour, DeadColour,
				                                                    (1 - (float)Enemy->CurrentHealth / Enemy->
					                                                    MaxHealth)));
			}
			else { SetActorLocation(TargetActor->GetActorLocation() + ActorOffset); }
		}
		else
		{
			...
			{
				Spotlight->SetLightColor(FLinearColor::LerpUsingHSV(HealthyColour, DeadColour,
				                                                    (1 - (float)Character->CurrentHealth / Character->
					                                                    MaxHealth)));
			}
		}
	}
