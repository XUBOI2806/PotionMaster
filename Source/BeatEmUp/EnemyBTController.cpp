...
void AEnemyBTController::BeginPlay()
{
	...
	BlackboardComponent->SetValueAsBool("HasTrap", true);
...
void AEnemyBTController::PlaceTrap()
{
	Cast<AEnemy>(GetPawn())->PlaceTrap();
	Trap--;
	BlackboardComponent->SetValueAsBool("HasTrap", Trap > 0);
}
