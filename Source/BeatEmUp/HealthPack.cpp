...
void AHealthPack::Tick(float DeltaTime)
{
	...
	if (Mesh->IsVisible())
	{
		...
		FVector NextLocation = FVector(InitialPosition.X, InitialPosition.Y,
		                               InitialPosition.Z + sin(GetGameTimeSinceCreation()) * 50);
		SetActorLocation(NextLocation);
		PointLight->SetIntensity(StartingIntensity * (CurrentLifeTime / MaxLifeTime));
		if (HealthMaterialInstance)
		{
			HealthMaterialInstance->SetScalarParameterValue(
				"Emissive", StartingEmissive * (CurrentLifeTime / MaxLifeTime));
			HealthMaterialInstance->SetScalarParameterValue("Transparency",
			                                                StartingTransparency * (CurrentLifeTime / MaxLifeTime));
		}
		if (CurrentLifeTime <= 0) { Destroy(); }
	}
}
